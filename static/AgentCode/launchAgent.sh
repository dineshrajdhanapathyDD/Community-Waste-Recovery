...
# Good Neighbor Agent - AgentCore Runtime Launch Script
...
print_header "Good Neighbor Agent - AgentCore Runtime Setup"
...
AGENT_ARN=$(COLUMNS=500 agentcore status --agent good_neighbor_agent 2>/dev/null | tr -d '\n' | sed -n 's/.*Agent ARN: \([^[:space:]]*\).*/\1/p')
...
# Step 7.5: Publish the frontend SPA to S3.
# The static site (index.html, app.js, styles.css) lives one level up from
# AgentCode in the `static/` directory. Upload it so CloudFront serves the app,
# then the config.js copies below overwrite the seeded stub with live values.
FRONTEND_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
if [ -f "$FRONTEND_DIR/index.html" ]; then
    print_status "Uploading frontend SPA from ${CYAN}${FRONTEND_DIR}${NC} to S3..."
    # Sync everything except the AgentCode backend dir and the local config.js
    # (config.js is copied separately below with the freshly generated values).
    aws s3 sync "$FRONTEND_DIR" "s3://${S3_BUCKET_NAME}/" \
        --exclude "AgentCode/*" \
        --exclude "config.js" \
        --exclude ".*"
    aws s3 sync "$FRONTEND_DIR" "s3://${S3_BUCKET_NAME}/public/" \
        --exclude "AgentCode/*" \
        --exclude "config.js" \
        --exclude ".*"
    print_success "Frontend SPA uploaded to S3"
else
    print_warning "No index.html found in $FRONTEND_DIR — skipping frontend upload"
fi
echo ""
